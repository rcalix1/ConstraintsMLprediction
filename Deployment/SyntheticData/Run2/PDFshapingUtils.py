...
class PDFshapingUtils:
    ...
    def get_upper_lower_bounds_constraints(self):
        mu  = self.y_means.to(self.device).reshape(-1)        # [3]
        std = self.y_deviations.to(self.device).reshape(-1)   # [3]

        # Correlation from dataset (fta, hmt, tgt) — numeric & no NaNs
        cols = ['o_fta', 'o_hmt', 'o_tgt']
        ...
        k = torch.empty(self.batch_size_syn, 3, device=self.device, dtype=mu.dtype).uniform_(0.2, 1.2)
